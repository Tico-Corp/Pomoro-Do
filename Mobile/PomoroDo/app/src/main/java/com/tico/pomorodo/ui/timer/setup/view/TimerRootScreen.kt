...
fun TimerRootScreen(
    ...
    Column(
        ...
        CustomTextButton(
            ...
            if (concentrationTimeInMinute == 0 && breakTimeInMinute == 0) {
                Toast.makeText(
                    context,
                    context.getString(R.string.toast_set_timer),
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                runBlocking {
                    setState(concentrationTimeInMinute, breakTimeInMinute)
                    if (concentrationTimeInMinute != 0) navigateToConcentrationMode()
                    else navigateToBreakMode()
