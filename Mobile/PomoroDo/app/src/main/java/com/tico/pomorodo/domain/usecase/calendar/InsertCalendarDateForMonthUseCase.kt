...
class InsertCalendarDateForMonthUseCase @Inject constructor(private val calendarRepository: CalendarRepository) {
    suspend operator fun invoke(calendarDate: CalendarDate) {
        calendarRepository.insertCalendarDateForMonth(calendarDate)
    }
}